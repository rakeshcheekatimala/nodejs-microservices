...
class ServiceClient {
    static async getServiceName(servicename) {
        try {
    ...
    static async callService(servicename,requestOptions) {
        const {ip,port} = await this.getServiceName(servicename);
        ...
        try {
           ...
           return response.data;
            ...
            const errorMessage =  (error.response && error.response.data && error.data.message) ||
            error.message;
            throw new Error(errorMessage);
        }
